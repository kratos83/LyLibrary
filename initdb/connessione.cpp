...
void connessione::creadb(QSqlDatabase db, QString db_lylibrary, int year){
        ...
        {
           QMessageBox MsgBox1;
           MsgBox1.setText(QString::fromLatin1(tr("Errore di connessione al DB").toStdString().c_str()));
           MsgBox1.setInformativeText(QString::fromLatin1(tr("Controllare di aver installato Mysql e di aver creato il DB lylibrary").toStdString().c_str()));
...
void connessione::connettidatabase(){
    ...
    if(ip_dat->text().length() == 0 || n_dat_lan->text().length() == 0 || user->text().length() == 0 || pwd_lan->text().length() == 0){
        QMessageBox MsgBox2;
        MsgBox2.setText(QString::fromLatin1(tr("Errore di connessione al DB").toStdString().c_str()));
        MsgBox2.setInformativeText(QString::fromLatin1(tr("Verificare che i dati siano corretti").toStdString().c_str()));
    ...
    if (db.open())
    {
        ...
        QMessageBox MsgBox2;
        MsgBox2.setText(QString::fromLatin1(tr("Errore di connessione al DB").toStdString().c_str()));
        MsgBox2.setInformativeText(QString::fromLatin1(tr("Impossibile connettersi al db.Controllare le impostazioni.").toStdString().c_str()));
        MsgBox2.setWindowTitle(tr("LyLibrary"));
        QIcon icon;
        icon.addFile(QString::fromLatin1(":/images/logo1.png"), QSize(), QIcon::Normal, QIcon::Off);
        MsgBox2.setWindowIcon(icon);
