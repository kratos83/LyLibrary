...
void fattura_rg_art::prezzo_senza_iva()
{
    if(f_libri->isChecked()){
        QSqlQuery Query1;
        Query1.prepare("select sum(prezzo_s_iva) from fatture_righe_vendita_art where id ='"+id_fatt->text()+"'");
        Query1.exec();
        if(Query1.next()){
        ...
        QSqlQuery Query1;
        Query1.prepare("select sum(prezzo_s_iva) from fatture_righe_vendita_prod_dig where id ='"+id_fatt->text()+"'");
        Query1.exec();
        if(Query1.next()){
            fatt->imponibile_2->setText(Query1.value(0).toString());
        }
...
void fattura_rg_art::iva_sum()
{
    if(f_libri->isChecked()){
        QSqlQuery Query2;
        Query2.prepare("select sum(prezzo_c_iva-prezzo_s_iva) from fatture_righe_vendita_art where id='"+id_fatt->text()+"'");
        Query2.exec();
        if(Query2.next()){
        ...
        QSqlQuery Query2;
        Query2.prepare("select sum(prezzo_c_iva-prezzo_s_iva) from fatture_righe_vendita_prod_dig where id='"+id_fatt->text()+"'");
        Query2.exec();
        if(Query2.next()){
            fatt->iva_ft_2->setText(Query2.value(0).toString());
        }
...
void fattura_rg_art::prezzo_con_iva()
{
    if(f_libri->isChecked()){
        QSqlQuery Query3;
        Query3.prepare("select sum(totale) from fatture_righe_vendita_art where id ='"+id_fatt->text()+"'");
        Query3.exec();
        if(Query3.next()){
        ...
        QSqlQuery Query3;
        Query3.prepare("select sum(totale) from fatture_righe_vendita_prod_dig where id ='"+id_fatt->text()+"'");
        Query3.exec();
        if(Query3.next()){
        fatt->totale_2->setText(Query3.value(0).toString());
        }
...
void fattura_rg_art::inserisci_riga_art()
{
    ...
    QSqlQuery Qctrl;
    Qctrl.prepare("SELECT * FROM fatture_righe_vendita_art WHERE cod_art = :cod_art");
...
void fattura_rg_art::inserisci()
{
    //Tentativo di inserimento record perché nuovo codice voce
                 ...
                 if(f_libri->isChecked() == true){
                     ...
                     QSqlQuery Query;
                     Query.prepare("INSERT INTO fatture_righe_vendita_prod_dig (id,cod_art,codbarre,nome_articolo,descrizione,unita,prezzo_unit,prezzo_s_iva,prezzo_c_iva,iva,quantita,sconto,totale)"
                                   " VALUES (:id, :cod_art, :codbarre, :nome_articolo, :descrizione,:unita,:prezzo_unit,:prezzo_s_iva,:prezzo_c_iva,:iva,:quantita,:sconto,:totale)"
                                 );
                     Query.bindValue(":id",id_fatt->text());
                     Query.bindValue(":cod_art",cod_art->text());
                     ...
                     QSqlQuery query("select quantita,pr_unit,pr_s_iva,pr_c_iva,totale from carico_prod_dig where cod_articolo='"+cod_art->text()+"'");
                     QSqlQuery query1("select quantita,pr_unit,pr_s_iva,pr_c_iva,totale from scarico_prod_dig where cod_articolo='"+cod_art->text()+"'");
                     query.exec();
                     query1.exec();
                     if(query.next()){
                     found = true;
                         ...
                         double tot_ = query.value(4).toDouble();
                             update_prod_dig(qa);
                             update_carico_prod_dig(qa,pr,pr_s,pr_c,tot_);
                             if(query1.next()){
                                 double qa = query1.value(0).toDouble();
                                 double pr = query1.value(1).toDouble();
                                 double pr_s = query1.value(2).toDouble();
                                 double pr_c = query1.value(3).toDouble();
                                 double tot_ = query1.value(4).toDouble();
                                 update_scarico_prod_dig(qa,pr,pr_s,pr_c,tot_);
                             }
                             else{
                                 insert_scarico_prod_dig();
                             }
                     }
                     if (Query.exec()) //Se esito OK inserimento DB
                     {
...
void fattura_rg_art::insert_scarico_prod_dig()
{
    QSqlQuery Query1;
    Query1.prepare("INSERT INTO scarico_prod_dig (cod_articolo,codbarre,nome_articolo,descrizione,autore,lingua,infoeditore,categ,scaffale,quantita,pr_unit,pr_s_iva,pr_c_iva,totale,image)"
                   " VALUES (:cod_articolo,:codbarre,:nome_articolo,:descrizione,:autore,:lingua,:infoeditore,:categ,:scaffale,:quantita,:pr_unit,:pr_s_iva,:pr_c_iva,:totale,:image)");
...
void fattura_rg_art::aggiorna_riga()
{
    ...
    close();
        ...
        QSqlQuery Query;
        Query.prepare("UPDATE fatture_righe_vendita_prod_dig SET  id = :id, nome_articolo=:nome_articolo, "
                      "prezzo_s_iva=:prezzo_s_iva, descrizione=:descrizione, unita=:unita, prezzo_unit=:prezzo_unit, "
                      "prezzo_c_iva=:prezzo_c_iva, iva=:iva, quantita=:quantita, sconto=:sconto, totale=:totale "
                      "WHERE cod_art = :cod_art");
        Query.bindValue(":id",id_fatt->text());
        ...
        formattanum();
        Query.bindValue(":totale",QString::number(set));
        QSqlQuery query1("select quantita,pr_unit,pr_s_iva,pr_c_iva,totale from carico_prod_dig where cod_articolo='"+cod_art->text()+"'");
        if(query1.next()){
        found = true;
        ...
        double tot_ = query1.value(4).toDouble();
        update_prod_dig(qa1);
        update_scarico_prod_dig(qa1,pr,pr_s,pr_c,tot_);
...
void fattura_rg_art::update_prod_dig(double qad)
{
    QSqlQuery Query2;
...
void fattura_rg_art::update_carico_prod_dig(double pro, double _pr_u, double _p_siva, double _p_civa, double _tot)
{
    QSqlQuery Query2;
...
void fattura_rg_art::update_scarico_prod_dig(double pro, double _pr_u, double _p_siva, double _p_civa, double _tot)
{
    QSqlQuery Query2;
    Query2.prepare("UPDATE scarico_prod_dig  SET codbarre=:codbarre, nome_articolo=:nome_articolo, "
                  " descrizione=:descrizione, autore=:autore, lingua=:lingua, infoeditore=:infoeditore, "
                  "categ=:categ, scaffale=:scaffale, quantita=:quantita,  pr_unit=:pr_unit, pr_s_iva=:pr_s_iva, "
                   "pr_c_iva=:pr_c_iva, totale=:totale, image=:image "
                  "WHERE cod_articolo = :cod_articolo");
