    ...
    QWidget(parent)
{
    ...
    agg_fattura_prod_dig = new QAction("Fattura prodotti digitali",this);

    /**********************************************
     * Impostazione menu pushbutton Aggiungi
     **********************************************/
    menu = new QMenu(this);
     ...
     *IMPOSTAZIONE AZIONI PUSHBUTTON
     *********************************************/
    el_libri = new QAction("Elimina fattura libri",this);
    el_prod_dig = new QAction("Elimina fattura prodotti digitali",this);

    /**********************************************
     * Impostazione menu pushbutton elimina
     **********************************************/
    menu2 = new QMenu(this);
    menu2->addAction(el_libri);
    menu2->addSeparator();
    menu2->addAction(el_prod_dig);
    elimina->setMenu(menu2);

    /**********************************************
     *IMPOSTAZIONE AZIONI PUSHBUTTON MODIFICA
     *********************************************/
    mod_fat_libri = new QAction("Modifica fattura libri",this);
    mod_fat_prod_dig = new QAction("Modifica fattura prodotti digitali",this);

    /**********************************************
     * Impostazione menu pushbutton modifica
     **********************************************/
    menu1 = new QMenu(this);
    menu1->addAction(mod_fat_libri);
    menu1->addSeparator();
    menu1->addAction(mod_fat_prod_dig);
    modifica->setMenu(menu1);

    /**********************************************
     *IMPOSTAZIONE AZIONI PUSHBUTTON MODIFICA
     *********************************************/
    print_libri = new QAction("Stampa fattura libri",this);
    print_prod_dig = new QAction("Stampa fatture prodotti digitali",this);

    /**********************************************
     * Impostazione menu pushbutton modifica
     **********************************************/
    menu3 = new QMenu(this);
    menu3->addAction(print_libri);
    menu3->addSeparator();
    menu3->addAction(print_prod_dig);
    p_fatt_s->setMenu(menu3);

    /**********************************************
    *IMPOSTAZIONE AZIONI PUSHBUTTON ESPORTA PDF
    *********************************************/
    ex_pdf_libri = new QAction("Esporta fattura libri in pdf",this);
    ex_pdf_prod_diag = new QAction("Esporta fattura prodotti digitali in pdf",this);

    /**********************************************
    * Impostazione menu pushbutton esporta pdf
    **********************************************/
    menu4 = new QMenu(this);
    menu4->addAction(ex_pdf_libri);
    menu4->addSeparator();
    menu4->addAction(ex_pdf_prod_diag);
    pushButton->setMenu(menu4);
    ...
    connect(agg_fattura_libri,SIGNAL(triggered()),this,SLOT(nuova_fattura_vendita_libri()));
    connect(agg_fattura_prod_dig,SIGNAL(triggered()),this,SLOT(nuova_fattura_vendita_prodotti_digitali()));
    connect(ck_lb,SIGNAL(clicked(bool)),this,SLOT(clicca_check()));
    connect(ck_pr,SIGNAL(clicked(bool)),this,SLOT(clicca_true()));
    ...
    disconnetti_button_prod_dig();
}

bool fattura::clicca_check()
{
    if(ck_lb->isChecked())
    {
        connect(mod_fat_libri,SIGNAL(triggered()),this,SLOT(aggiorna_fattura_libri()));
        connect(el_libri,SIGNAL(triggered()),this,SLOT(elimina_fattura()));
        connect(tab_view,SIGNAL(clicked(QModelIndex)),this,SLOT(clickgrid_libri()));
        connect(cerca_txt,SIGNAL(textChanged(QString)),this,SLOT(cerca_dxt_libri(const QString &)));
        connect(print_libri,SIGNAL(triggered()),this,SLOT(stampa_libri()));
        connect(ex_pdf_libri,SIGNAL(triggered()),this,SLOT(esporta_pdf_libri()));
        lista_libri();
        vis_dati_fattura_libri();
        disconnetti_button_prod_dig();
    }

    return ck_pr->isChecked();
}

void fattura::disconnetti_button_prod_dig()
{
    disconnect(mod_fat_prod_dig,SIGNAL(triggered()),this,SLOT(aggiorna_fattura_prod_dig()));
    disconnect(el_prod_dig,SIGNAL(triggered()),this,SLOT(elimina_fattura_prod_dig()));
    disconnect(print_prod_dig,SIGNAL(triggered()),this,SLOT(stampa_prod_dig()));
    disconnect(ex_pdf_prod_diag,SIGNAL(triggered()),this,SLOT(esporta_pdf_prod_dig()));
    ck_pr->setChecked(false);
    mod_fat_prod_dig->setEnabled(false);
    el_prod_dig->setEnabled(false);
    print_prod_dig->setEnabled(false);
    agg_fattura_prod_dig->setEnabled(false);
    mod_fat_libri->setEnabled(true);
    el_libri->setEnabled(true);
    agg_fattura_libri->setEnabled(true);
    ex_pdf_libri->setEnabled(true);
    ex_pdf_prod_diag->setEnabled(false);
    print_libri->setEnabled(true);
}

bool fattura::clicca_true()
{
    if(ck_pr->isChecked())
    {
        connect(mod_fat_prod_dig,SIGNAL(triggered()),this,SLOT(aggiorna_fattura_prod_dig()));
        connect(el_prod_dig,SIGNAL(triggered()),this,SLOT(elimina_fattura_prod_dig()));
        connect(cerca_txt,SIGNAL(textChanged(QString)),this,SLOT(cerca_dxt_libri(const QString &)));
        connect(tab_view,SIGNAL(clicked(QModelIndex)),this,SLOT(clickgrid_libri()));
        connect(print_prod_dig,SIGNAL(triggered()),this,SLOT(stampa_prod_dig()));
        connect(ex_pdf_prod_diag,SIGNAL(triggered()),this,SLOT(esporta_pdf_prod_dig()));
        lista_prod_dig();
        vis_dati_fattura_prod_dig();
        disconnetti_button_libri();
    }

    return ck_lb->isChecked();
}

void fattura::disconnetti_button_libri()
{
    disconnect(mod_fat_libri,SIGNAL(triggered()),this,SLOT(aggiorna_fattura_libri()));
    disconnect(el_libri,SIGNAL(triggered()),this,SLOT(elimina_fattura()));
    disconnect(print_libri,SIGNAL(triggered()),this,SLOT(stampa_libri()));
    disconnect(ex_pdf_libri,SIGNAL(triggered()),this,SLOT(esporta_pdf_libri()));
    ck_lb->setChecked(false);
    mod_fat_libri->setEnabled(false);
    el_libri->setEnabled(false);
    agg_fattura_libri->setEnabled(false);
    mod_fat_prod_dig->setEnabled(true);
    el_prod_dig->setEnabled(true);
    agg_fattura_prod_dig->setEnabled(true);
    ex_pdf_prod_diag->setEnabled(true);
    print_libri->setEnabled(false);
    ex_pdf_libri->setEnabled(true);
    print_prod_dig->setEnabled(true);
}
...
void fattura::cerca_dxt_prod_dig(const QString &)
{
    if(cerca_txt->currentIndex() == 0){
    QString text1;
    text1 = cerca1->text();
...
void fattura::stampa_fat(){
    ...
    ft.setWindowTitle("Anteprima di stampa.");
...
void fattura::clickgrid_prod_dig()
{
    QString id_new;
...
void fattura::nuova_fattura_vendita_libri(){
    ...
    fs->f_libri_2->setChecked(true);
    fs->f_prod_dig_2->setEnabled(false);
    fs->show();
    }
    else{
...
void fattura::nuova_fattura_vendita_prodotti_digitali()
{
    ...
    if(query.next()){
        if(query.value(0).toInt() == 0){
            fs->id1->setText("1");
            fs->formatta_num();
    connect(fs,SIGNAL(salvafattura()),SLOT(lista_prod_dig()));
    fs->f_prod_dig_2->setChecked(true);
    fs->f_libri_2->setEnabled(false);
    fs->show();
    }
    else{
            fs->id1->setText(query.value(0).toString());
...
void fattura::aggiorna_fattura_prod_dig()
{
...
void fattura::agg_fatt_prod_dig(QModelIndex index)
{
    if(index.row() < mod_grid->rowCount()){
        fattura_rg *ft = new fattura_rg(this);
        QSqlQuery query;
        query.prepare("select * from fatture_vendita_righe_dig where id='"+id->text()+"'");
        query.exec();
        ft->setModal(true);
        ...
        if(rec.value("tipo_fattura").toString() == "Prodotti digitali"){
            ft->f_libri_2->setEnabled(false);
            ft->f_prod_dig_2->setChecked(true);
        }
...
void fattura::agg_fatt_libri(QModelIndex index){
    if(index.row() < mod_grid->rowCount()){
            ...
            if(rec.value("tipo_fattura").toString() == "Vendita libri"){
                ft->f_prod_dig_2->setEnabled(false);
                ft->f_libri_2->setChecked(true);
            }
            ft->sconto1->setText(query.value(4).toString());
            ft->sconto2->setText(query.value(5).toString());
            ft->sconto3->setText(query.value(6).toString());
...
void fattura::vis_dati_fattura_libri(){
    ...
    if(query.next()){
        ...
        QMessageBox::warning(this,"LyLibrary","Impossibile ricavare le fatture di vendita...");
    }
...
void fattura::vis_dati_fattura_prod_dig()
{
    QSqlQuery query,query1;
    query.prepare("select count(id) from fattura_vendita");
    query.exec();
...
void fattura::elimina_fattura_prod_dig()
{
    if (!id->text().isEmpty())
    {
        ...
        QSqlQuery qctrl;
        qctrl.prepare("SELECT * FROM fattura_vendita WHERE id = :id");
...
void fattura::elimina_riga_pr_dig()
{
    // Se si passano i controlli di cui sopra si procede all'eliminazione

    QSqlQuery query,query1,query2;

    query.prepare("DELETE FROM fattura_vendita WHERE id = :id");
    query.bindValue(":id",id->text());

    query1.prepare("DELETE FROM fatture_righe_vendita_prod_dig WHERE id = :id");
    query1.bindValue(":id",id->text());

    query2.prepare("DELETE FROM fatture_vendita_righe_dig where id=:id");
    ...
    else
    {
        ...
        QMessageBox::warning(this,"LyLibrary","Impossibile eliminare il record "+id->text());
     }
    lista_prod_dig();
    vis_dati_fattura_prod_dig();
    textEdit->setText("");
...
void fattura::elimina_riga(){
    ...
    query1.prepare("DELETE FROM fatture_righe_vendita_art WHERE id = :id");
    query1.bindValue(":id",id->text());

    query2.prepare("DELETE FROM fatture_vendita_righe where id=:id");
    query2.bindValue(":id",id->text());
...
void fattura::cerca_libri(){
    ...
    if(query.next()){
        ...
        QMessageBox::warning(this,"LyLibrary","Impossibile trovare le informazioni...");
    }
}

void fattura::cerca_prod_dig()
{
    QSqlQuery query;
    query.prepare("select * from fattura_vendita where id=:id");
    query.bindValue(":id",id->text());
...
void fattura::stampa_libri()
{
        ...
        preview.setWindowTitle("Anteprima di stampa fattura libri.");
        QIcon icon;
        ...
        preview.exec();
            ...
            QMessageBox::warning(this,"LyLibrary","Selezionare una fattura da stampare...");
        }
}

void fattura::stampa_prod_dig()
{
    if(tab_view->selectionModel()->currentIndex().isValid()){
    QPrinter printer(QPrinter::HighResolution);
    QPrintPreviewDialog preview(&printer);
    preview.setWindowFlags(Qt::Window);
    preview.setWindowTitle("Anteprima di stampa fattura prodotti digitali.");
    QIcon icon;
    icon.addFile(QString::fromUtf8(":/images/document-preview.png"), QSize(), QIcon::Normal, QIcon::Off);
    preview.setWindowIcon(icon);
    connect(&preview,SIGNAL(paintRequested(QPrinter*)),SLOT(preview_prod_dig(QPrinter*)));
    preview.exec();
    }
    else{
        QMessageBox::warning(this,"LyLibrary","Selezionare una fattura da stampare...");
...
void fattura::preview_prod_dig(QPrinter *printer){
    ...
    tab_view->selectRow(riga);
    Stampe *pr = new Stampe();
    pr->stampa_fatt_vendita_prod_dig(printer,mod_grid,newindex,id->text().toDouble());
}
...
void fattura::esporta_pdf_libri()
{
    ...
    if (fileName.length() != 0) {
       ...
       st->stampa_fatt_vendita_libri(printer,mod_grid,tab_view->currentIndex(),id->text().toDouble());
        ...
        QMessageBox::warning(this,"LyLibrary","Selezionare una riga da esportare");
    }

}

void fattura::esporta_pdf_prod_dig()
{
    if(tab_view->selectionModel()->currentIndex().isValid()){
    QString fileName = QFileDialog::getSaveFileName(this, "Esporta PDF",
                                                    "*.pdf", "Pdf Files(*.pdf);;Tutti i file(*.*)");
    ...
    if (fileName.length() != 0) {
       ...
       printer->setOutputFileName(fileName);
       Stampe *st = new Stampe();
       st->stampa_fatt_vendita_prod_dig(printer,mod_grid,tab_view->currentIndex(),id->text().toInt());
    }
    else{
        QMessageBox::warning(this,"LyLibrary","Selezionare una riga da esportare");
    }
